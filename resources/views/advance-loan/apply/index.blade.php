            ...
            <div class="col-6 form-group">
                <select class="form-control" id="advance_type" name="advance_type">
                    ...
                    @foreach ($advanceTypes as $type)
                        <option value="{{ $type->id }}" {{ request()->get('advance_type') == $type->id ? 'selected' : '' }}>
                            {{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
                                                ...
                                                <tbody>
                                                    @forelse($advances as $advance)
                                                        <tr>
                                                            ...
                                                            <td>{{ \Carbon\Carbon::parse($advance->transaction_date)->format('d-M-Y') }}
                                                            </td>
                                                            <td class="text-right">{{ formatAmount($advance->amount) }}</td>
                                                            <td class="text-center">
                                                                {{ \Carbon\Carbon::parse($advance->created_at)->format('d-M-Y') }}
                                                                at
                                                                {{ \Carbon\Carbon::parse($advance->created_at)->format('h:i A') }}
                                                            </td>
                                                            <td class="text-center">
                                                                @php
                                                                    $statusClasses = [
