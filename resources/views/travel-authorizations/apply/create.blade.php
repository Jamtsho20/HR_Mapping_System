...
document.querySelectorAll('.from_date').forEach(function(fromDateField) {
    fromDateField.addEventListener('change', function() {
        var fromDate = this.value;
        var toDateField = this.closest('tr').querySelector('.to_date'); // Find the corresponding "to_date" field within the same row
        ...
        if (fromDate) {
            ...
            toDateField.value = ''; // Clear the value when disabled
        }
    });
});
...
document.querySelector('.add-row').addEventListener('click', function(e) {
    ...
    const newRow = document.createElement('tr');
        newRow.innerHTML = `
            ...
            <td>
                <input type="date" id="to_date" name="details[${rowCount}][to_date]" class="form-control form-control-sm to_date ">
            </td>
            <td>
