@extends('layouts.app')
@section('page-title', 'Other Pay Changes')
@section('content')
    @if ($privileges->create)
        @section('buttons')
            <a href="{{ route('other-pay-changes.create') }}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> New Pay
                Slip</a>
        @endsection
    @endif
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Other Pay Changes</h3>
                    </div>
                    <div class="card-body">
                                                                    ...
                                                                    <td class="text-center">
                                                                        @if ($privileges->edit)
                                                                            @if ($record->status['key'] == 1)
                                                                                <a href="{{ route('other-pay-changes.show', $record->id) }}"
                                                                                    class="btn btn-sm btn-rounded btn-outline-success">
                                                                                    <i class="fa fa-spinner"></i>
                                                                                    PROCESS
                                                                                </a>

                                                                                <a href="{{ route('other-pay-changes.finalize', $record->id) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#finalize-btn').on('click', function() {
                ...
                $.confirm({
                    title: 'Approve Pay Changes',
                    content: "Are you sure to approve the these Pay Changes? This will change employee's Basic Pay and cannot be undone.",
                    buttons: {
                        confirm: {
                            text: 'Yes, I confirm',
