...
<div class="row">
    ...
    <div class="col-sm-4 ">
        <input type="checkbox" id="chkAttachmentRequired" name="leave_plan[attachment_required]" value="1"
            {{ old('leave_policy.attachment_required') ? 'checked' : '' }}>
    </div>
</div>
    ...
    <div class="col-sm-4">
        <select class="form-control" id="gender" name="leave_plan[gender]" required>
            ...
            @foreach (config('global.gender_with_all') as $key => $value)
                <option value="{{ $key }}" {{ old('leave_plan.gender') == $key ? 'selected' : '' }}>
                    {{ $value }}</option>
            @endforeach
        </select>
    </div>
                ...
                <div class="form-check">
                    <label class="form-check-label" style="font-weight: 400;">
                        <input type="checkbox" id="chkavailprobationperiad" name="leave_plan[can_avail_in][]"
                            ...
                            {{ old('leave_plan.can_avail_in') ? 'checked' : '' }}>&nbsp;{{ $employmentType->name }}
                    </label>
                </div>
            </div>
        @endif
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="text" min="0" maxlength="5"
                                    name="leave_policy_rule[AAAAA][duration]" class="form-control resetKeyForNew"
                            ...
                            <td class="text-center">
                                <select class="form-control resetKeyForNew" id="UOM"
                                    name="leave_policy_rule[AAAAA][uom]">
                                    <option value="" disabled selected hidden>Select your option</option>
                                    <option value="1"
                                        {{ old('leave_policy_rule.AAAAA.uom') == 1 ? 'selected' : '' }}>Day</option>
                                    <option value="2"
                                        {{ old('leave_policy_rule.AAAAA.uom') == 2 ? 'selected' : '' }}>Month</option>
                                    <option value="3"
                                        {{ old('leave_policy_rule.AAAAA.uom') == 3 ? 'selected' : '' }}>Year</option>
                                </select>
                            </td>

                            <td class="text-center">
                                <input type="date" placeholder="dd-mmm-yyyy"
                                    class="resetKeyForNew form-control mycal hasDatepicker"
                            ...
                            <td class="text-center">
                                <select class="form-control resetKeyForNew" id="ddlislossofpay"
                                    name="leave_policy_rule[AAAAA][is_loss_of_pay]">
                                    <option value="" disabled
                                        {{ old('leave_policy_rule.AAAAA.is_loss_of_pay') === null ? 'selected' : '' }}
                                        hidden>Select your option</option>
                                    <option value="1"
                                        {{ old('leave_policy_rule.AAAAA.is_loss_of_pay') === '1' ? 'selected' : '' }}>
                                        Yes</option>
                                    <option value="0"
                                        {{ old('leave_policy_rule.AAAAA.is_loss_of_pay') === '0' ? 'selected' : '' }}>
                                        No</option>
                                </select>
                            </td>
                            ...
                            <td class="text-center">
                                <select class="form-control resetKeyForNew" id="Employeetype"
                                    name="leave_policy_rule[AAAAA][mas_employment_type_id]">
                                    <option value="" disabled selected hidden>Select your option</option>
                                    @foreach ($employmentTypes as $employmentType)
                            ...
                            <td class="text-center">
                                <select class="form-control resetKeyForNew" name="leave_policy_rule[AAAAA][status]">
                                    <option value="" disabled
                                        {{ old('leave_policy_rule.AAAAA.status') === null ? 'selected' : '' }}>Select
                                        your option</option>
                                    @foreach (config('global.status') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('leave_policy_rule.AAAAA.status') == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <tr class="notremovefornew">
                            ...
                            <td class="text-right">
                                <a href="#" class="add-table-row btn btn-sm btn-info"
                                    style="font-size: 12px"><i class="fa fa-plus"></i> Add New Row</a>
                            </td>
                        </tr>
