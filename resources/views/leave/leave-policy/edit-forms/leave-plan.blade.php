...
<div class="row">
    ...
    <div class="col-sm-4">
        <select class="form-control" id="credit-frequency" name="leave_plan[credit]">
            <option value="" disabled selected hidden>Select your option</option>
            <option value="1" {{$leavePolicy->leavePolicyPlan->leave_plan==1 ? 'selected' : '' }}>Monthly</option>
            <option value="2" {{$leavePolicy->leavePolicyPlan->leave_plan==2 ? 'selected' : '' }}>Yearly</option>
        </select>
    </div>
</div>
<!-- leave limit -->
...
<div class="row">
    ...
    <div class="col-sm-2">
        <div class="form-check">
            <label class="form-check-label" style="font-weight: 400;">
                <input type="checkbox" id="chkavailprobationperiad" name="leave_plan[can_avail_in][]" value="{{$employmentType->id}}" class="can_avail" {{ $leavePolicy->leavePolicyPlan->can_avail_in? 'checked' : '' }}>&nbsp;{{$employmentType->name}}
            </label>
        </div>
    </div>
    @endforeach
                ...
                <table id="leave-rules" class="table table-condensed table-bordered table-striped table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="text" min="0" maxlength="5" name="leave_policy_rule[AAAAA][duration]" class="form-control resetKeyForNew" id="duration" placeholder="Duration" value="{{$rule->duration}}">
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                <input type="date" placeholder="dd-mmm-yyyy" class="form-control mycal hasDatepicker resetKeyForNew" style="background-color: rgb(255, 255, 255);" name="leave_policy_rule[AAAAA][start_date]" value="{{$rule->start_date}}">
                            </td>
                            <td class="text-center">
                                <input type="date" placeholder="dd-mmm-yyyy" class="form-control mycal hasDatepicker resetKeyForNew" style="background-color: rgb(255, 255, 255);" name="leave_policy_rule[AAAAA][end_date]" value="{{$rule->end_date}}">
                            </td>
                            <td class="text-center">
