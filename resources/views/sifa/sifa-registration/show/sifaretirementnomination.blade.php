<label for=""><strong>SIFA Retirement And Nominations</strong></label>
<small>(s)<i> (I hereby nominate the person(s) mentioned below, who is/are member(s) of my family, to have the conferred right to claim the retirement benefit upon my demise, as per the percentage of shares prescribed)</i></small>
<br><br>
...
<div class="table-responsive criteria">
    <table id="sifa_retirement_and_nomination" class="table table-condensed table-striped table-bordered table-sm">
        ...
        <tbody>
            @if($sifaRegistration->sifaRetirementAndNomination && $sifaRegistration->sifaRetirementAndNomination->isNotEmpty())
                @foreach($sifaRegistration->sifaRetirementAndNomination as $key => $nomination)
                    <tr>
                ...
                @endforeach
            @else
                <!-- Default empty row when no nominations -->
                <tr>
