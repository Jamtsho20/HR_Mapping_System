<div class="tab-pane">
     <div class="row border" >
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <div class="d-flex" style="gap:4px">
                            ...
                            <select id="step_point" name="job[step_point]" class="form-control form-control-sm" style="width:50%">
                                <option value="" disabled selected hidden>Select point</option>
                                @if(!empty($points))
                                    @foreach($points as $point)
                                        <option value="{{ $point }}" {{ $point === $selectedPoint ? 'selected' : '' }}>{{ $point }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="text" id="pay_scale" name="job[basic_pay]" class="form-control form-control-sm" value="{{old('job.pay_scale',isset($employee->empJob->gradeStep->pay_scale) ? $employee->empJob->gradeStep->pay_scale : '')}}" required disabled>
                </div>
                <div class="form-group col-md-4">
                    <label for="basic_pay">Basic Pay<span class="text-danger">*</span></label>
                    <input type="text" name="job[basic_pay]" id="basic_pay" class="form-control form-control-sm" value="{{ old('job.basic_pay',isset($employee->empJob->basic_pay) ? $employee->empJob->basic_pay:'') }}" required readonly>
                </div>
                <div class="form-group col-md-4">
...
@push('page_scripts')
    <script>
        ...
        document.getElementById('grade_step_id').addEventListener('change', function() {
            ...
            stepPointDropdown.innerHTML = "<option value='' disabled selected hidden>Select point</option>"; // Reset the point dropdown
