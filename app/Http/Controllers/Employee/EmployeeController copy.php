...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->permenant_address) {
            $permenantAddress = modifyFormRequest($request->permenant_address);
        }
        if ($request->current_address) {
            $currentAddress = modifyFormRequest($request->current_address);
        }
        if ($request->job) {
            $job = modifyFormRequest($request->job);
        }
        if ($request->qualifications) {
            $qualification = modifyFormRequest($request->qualifications);
        }
        ...
        try {
            ...
            if (!empty($permenantAddress) && !empty($currentAddress)) {
                $this->saveAddress($permenantAddress, $currentAddress, $employeeId, null, $request);
            }
            if (!empty($job)) {
            ...
            if (!empty($qualification)) {
                ...
                $this->saveQualifications($qualification, $employeeId, $request);
            }
    ...
    private function savePersonalInfo($personalInfo, $id, $request)
    {
        ...
        if ($request->isMethod('put') || $request->isMethod('patch')) { //$request->isMethod('put') || $request->isMethod('patch') is for update
            ...
            $user->first_name = $personalInfo['first_name'] ?? null;
            $user->middle_name = $personalInfo['middle_name'] ?? null;
            $user->last_name = $personalInfo['last_name'] ?? null;
            $user->title = $personalInfo['title'] ?? null;
            $user->name = $fullName;
            $user->username = $userName;
            $user->password = bcrypt('password');
            $user->email = $personalInfo['email'] ?? null;
            $user->cid_no = $personalInfo['cid_no'] ?? null;
            $user->employee_id = $employeeId;
            $user->gender = $personalInfo['gender'] ?? null;
            $user->dob = $personalInfo['dob'] ?? null;
            $user->birth_place = $personalInfo['birth_place'] ?? null;
            $user->birth_country = $personalInfo['birth_country'] ?? null;
            $user->marital_status = $personalInfo['marital_status'] ?? null;
            $user->contact_number = $personalInfo['contact_number'] ?? null;
            $user->nationality = $personalInfo['nationality'] ?? null;
            $user->date_of_appointment = $personalInfo['date_of_appointment'] ?? null;
            $user->is_active = $personalInfo['is_active'] ?? null;
            $user->profile_pic = $profilePic;
            $user->cid_copy = $empCidCopy;
            $user->status = $request->status;
            $user->save();
    ...
    private function saveAddress($permanentAddress, $currentAddress, $employeeId, $id, $request)
    {
        ...
        if ((bool)$request->status) {
            $request->validate([
                ...
                'permanent_address.mas_dzongkhag_id' => 'required',
                'permanent_address.mas_gewog_id' => 'required',
                'permanent_address.mas_village_id' => 'required',
                'permanent_address.thram_no' => 'required',
                'permanent_address.house_no' => 'required',
    ...
    private function modifyFormRequest($formReques){
