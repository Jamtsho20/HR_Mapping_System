(function () {
    ...
    //suppose if the url has "system/users"
    if (position.length > 2) {
        var positionSegments = '/' + position[1] + '/' + position[2];
    } else { //this one only if the url has "dashboard"
        var positionSegments = '/' + position[1];
    }

    $(".app-sidebar li a").each(function () {
        var $this = $(this);
        var pageUrl = new URL($this.attr("href"));
        var pageUrlSegments = pageUrl.pathname;

        if (pageUrlSegments) {
            if (positionSegments == pageUrlSegments) {
